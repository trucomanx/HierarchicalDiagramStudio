...
class GraphicsNodeItem(QGraphicsItem):
    ...
    def contextMenuEvent(self, event):
        ...
        if t in (Node.NODE_NORMAL, Node.NODE_START, Node.NODE_END):
            act_edit = menu.addAction("✏ Editar título/resumo")
            act_edit.triggered.connect(self._edit_title_summary)

        if t == Node.NODE_NORMAL:
            menu.addSeparator()
            act_ai = menu.addAction("＋ Adicionar entrada")
            act_ai.triggered.connect(self._add_input)
            act_ri = menu.addAction("－ Remover última entrada")
            act_ri.triggered.connect(self._remove_input)
            act_ao = menu.addAction("＋ Adicionar saída")
            act_ao.triggered.connect(self._add_output)
            act_ro = menu.addAction("－ Remover última saída")
            act_ro.triggered.connect(self._remove_output)
            menu.addSeparator()
            if self.node.subdiagram:
                act_open = menu.addAction("🔗 Abrir subdiagrama")
                act_open.triggered.connect(self._open_subdiagram)
            else:
                act_create = menu.addAction("🆕 Criar subdiagrama")
        ...
        menu.addSeparator()
        act_rot = menu.addAction("↻ Rotacionar 90°")
        act_rot.triggered.connect(self._rotate)
        menu.addSeparator()
        act_del = menu.addAction("🗑 Deletar")
...
class GraphicsEdgeItem(QGraphicsPathItem):
    ...
    def _context_menu(self, event):
        menu = QMenu()
        act_del = menu.addAction("🗑 Deletar conexão")
        act_del.triggered.connect(self._delete)
        act_clr = menu.addAction("⟳ Limpar pontos intermediários")
...
class DiagramScene(QGraphicsScene):
    ...
    def contextMenuEvent(self, event):
        ...
        if not items:
            ...
            menu = QMenu()
            pos = event.scenePos()
            act_normal = menu.addAction("＋ Adicionar Bloco Normal")
            act_merge = menu.addAction("＋ Adicionar Bloco Merge")
            act_start = menu.addAction("＋ Bloco Start")
            act_end = menu.addAction("＋ Bloco End")
...
class EditNodeDialog(QDialog):
    def __init__(self, title: str, summary: str, show_summary: bool = True):
        ...
        self.summary_edit = QTextEdit(summary)
        self.summary_edit.setMaximumHeight(100)
        if show_summary:
            layout.addRow("Resumo:", self.summary_edit)
...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self) -> QWidget:
        ...
        def btn(icon: str, text: str, tip: str, cb) -> QPushButton:
        ...
        self.btn_new = btn("📄", "Novo", "Novo diagrama (Ctrl+N)", self.action_new)
        self.btn_open = btn("📂", "Abrir", "Abrir arquivo .diagram (Ctrl+O)", self.action_open)
        self.btn_save = btn("💾", "Salvar", "Salvar (Ctrl+S)", self.action_save)
        self.btn_save_as = btn("💾", "Salvar Como", "Salvar como...", self.action_save_as)
        ...
        self.btn_back = btn("⬅", "Voltar", "Voltar ao diagrama anterior", self.action_back)
    ...
    def _update_header(self):
        ...
        self.summary_label.setText(self.current_diagram.summary or "")
    ...
    def action_open(self):
        ...
        filepath, _ = QFileDialog.getOpenFileName(
            self, "Abrir Diagrama", "", "Diagram Files (*.diagram);;All Files (*)")
        if filepath:
    ...
    def action_save_as(self):
        filepath, _ = QFileDialog.getSaveFileName(
            self, "Salvar Diagrama Como", "", "Diagram Files (*.diagram);;All Files (*)")
        if filepath:
            if not filepath.endswith(".diagram"):
                filepath += ".diagram"
            self.current_filepath = filepath
    ...
    def create_subdiagram_for_node(self, node_item: GraphicsNodeItem):
        ...
        default_name = node.title.replace(" ", "_").lower() + ".diagram"
        filepath, _ = QFileDialog.getSaveFileName(
